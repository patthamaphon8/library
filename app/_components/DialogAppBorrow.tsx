...
import {
  ...
  returnBook,
} from "@/lib/action/borrowTransaction";
import { revalidate } from "@/lib/action/base";
import { motion } from "motion/react";
import { cn } from "@/lib/utils";

interface DialogBorrowReturnProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

const DialogBorrowReturn = ({
  open,
  onOpenChange,
}: DialogBorrowReturnProps) => {
  ...
  const handleOnDetect = async (value: string) => {
    ...
    if (response.status === "BORROWED") {
      setDefaultBorrowTransaction(response.id);
    }
  };

  const setDefaultBorrowTransaction = async (bookCopyId: number) => {
    const response = await getBorrowTransactionByBookCopyId(bookCopyId);
    console.log("🚀 ~ setDefaultBorrowTransaction ~ response:", response)
    if (!response) {
      return;
    }
    setBorrowDate(dayjs(response.borrowedAt));
    setDueDate(dayjs(response.dueDate));
    setUser(response.user);
    setBorrowTransaction(response);
  };
  ...
  const onSubmit = async () => {
    ...
    if (!dueDate) return;
    if (bookCopy.status === "BORROWED" && borrowTransaction) {
      console.log(borrowTransaction);
      const responseUpdate = await returnBook(borrowTransaction.id, dayjs().toISOString(), bookCopy.id);
      if(!responseUpdate){
        toast.error(`คืนหนังสือไม่สำเร็จ`, {
    ...
    <Dialog open={open} onOpenChange={handleOnOpenChange}>
      ...
      <DialogContent className={`min-w-[600px]`}>
        ...
        <div className="">
          ...
          <motion.div
            // animate={{
            //   height: bookCopy ? `auto` : `0px`,
            //   display: bookCopy ? `block` : `none`,
            //   transition: {
            //     delay: 1,
            //   }
            // }}
            animate={bookCopy ? "visable" : "hidden"}
            variants={{
              visable: {
                height: `auto`,
                // display: `block`,
          ...
          >
            <div className={`w-full bg-gray-300 my-4 rounded-full`}></div>
            <div className={`flex gap-4 mt-4`}>
              ...
              >
                <div
                  className={cn(
                    `relative flex-1`,
                    bookCopy?.status === "BORROWED" && `pointer-events-none`
                  )}
                >
                  <input
            ...
            <div className={`mt-4 relative`}>
              ...
              {user && (
                <div
                  className={`flex gap-4 items-center border border-gray-300 rounded-full bg-white h-10 absolute top-2 left-2 pl-3 pr-2`}
                >
                  <div
                    className={``}
                  >{`${user.firstName} ${user.lastName}`}</div>
                  <Button
                    disabled={bookCopy?.status === "BORROWED"}
                    onClick={() => {
                      setUser(undefined);
                    }}
                    variant={`outline`}
          ...
          <div className={`mt-4`}>
            ...
            >
              {bookCopy?.status === "AVAILABLE" ? "ยืม" : "คืน"}
            </Button>
          </div>
        </div>
      </DialogContent>
...
export default DialogBorrowReturn;
